...
<head>
    ...
    <title>Motorbikes Management | {{ config('app.name', 'Fleet Manager') }}</title>
...
<body>
        ...
        <div class="container px-4 px-lg-5">
            ...
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-5">
                ...
                <div class="d-flex gap-3">
                    ...
                    <a href="{{ route('motorbikes.create') }}" class="btn btn-gradient-primary">
                    ...
                    <div class="stat-card">
                        <div class="d-flex justify-content-between align-items-start mb-3">
                            <div>
                                ...
                                <h3 class="stat-value mb-0">{{ $totalBikes }}</h3>
                            </div>
                    ...
                    <div class="stat-card">
                        <div class="d-flex justify-content-between align-items-start mb-3">
                            <div>
                                ...
                                <h3 class="stat-value text-success mb-0">{{ $activeBikes }}</h3>
                            </div>
                        ...
                        <div class="progress-modern">
                            <div class="progress-bar-modern bg-success" style="width: {{ $totalBikes > 0 ? ($activeBikes / $totalBikes) * 100 : 0 }}%"></div>
                        </div>
                        <p class="small text-muted mt-2">{{ $totalBikes > 0 ? round(($activeBikes / $totalBikes) * 100) : 0 }}% of total fleet</p>
                    </div>
                </div>
                    ...
                    <div class="stat-card">
                        <div class="d-flex justify-content-between align-items-start mb-3">
                            <div>
                                ...
                                <h3 class="stat-value text-warning mb-0">{{ $maintenanceBikes }}</h3>
                            </div>
                    ...
                    <div class="stat-card">
                        <div class="d-flex justify-content-between align-items-start mb-3">
                            <div>
                                ...
                                <h3 class="stat-value text-info mb-0">{{ $availableBikes }}</h3>
                            </div>
                ...
                <div class="table-responsive">
                    <table class="modern-table table">
                        ...
                        <tbody id="tableBody">
                            ...
                            <tr class="motorbike-row" onclick="viewMotorbike({{ $bike->id }})">
                                <td>
                                    <div class="d-flex align-items-center gap-3">
                                        ...
                                        <div>
                                            <div class="fw-semibold">{{ $bike->plate_number }}</div>
                                            <div class="small text-muted">ID: {{ substr($bike->id, 0, 8) }}</div>
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <div class="fw-medium">{{ $bike->model }}</div>
                                    <div class="small text-muted">{{ $bike->year ?? 'Year N/A' }}</div>
                                </td>
                                <td>
                                    <code class="small">{{ $bike->engine_number }}</code>
                                </td>
                                <td>
                                ...
                                <td>
                                    <div class="small">{{ $bike->last_service_date ?? 'Not serviced' }}</div>
                                    @if($bike->last_service_date)
                                    <div class="small text-muted">
                                        {{ \Carbon\Carbon::parse($bike->last_service_date)->diffForHumans() }}
                                    </div>
                                    @endif
                                </td>
                                <td onclick="event.stopPropagation()">
                                    <div class="action-group">
                                        <button onclick="viewMotorbike({{ $bike->id }})" class="action-btn view" title="View Details">
                                            <i class="fas fa-eye"></i>
                                        </button>
                                        <a href="{{ route('motorbikes.edit', $bike) }}" class="action-btn edit" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <button onclick="deleteMotorbike({{ $bike->id }})" class="action-btn delete" title="Delete">
            ...
            <div class="pagination-modern">
                {{-- Previous Page Link --}}
                @if($motorbikes->onFirstPage())
                <span class="page-link-modern disabled opacity-50">
                    <i class="fas fa-chevron-left"></i>
                </span>
                @else
                <a href="{{ $motorbikes->previousPageUrl() }}" class="page-link-modern">
                    <i class="fas fa-chevron-left"></i>
                </a>
                @endif

                {{-- Pagination Elements --}}
                @foreach(range(1, $motorbikes->lastPage()) as $i)
                    @if($i >= $motorbikes->currentPage() - 2 && $i <= $motorbikes->currentPage() + 2)
                        @if($i == $motorbikes->currentPage())
                        <span class="page-link-modern active">{{ $i }}</span>
                        @else
                        <a href="{{ $motorbikes->url($i) }}" class="page-link-modern">{{ $i }}</a>
                        @endif
                    @endif
                @endforeach

                {{-- Next Page Link --}}
                @if($motorbikes->hasMorePages())
                <a href="{{ $motorbikes->nextPageUrl() }}" class="page-link-modern">
    ...
    <script>
        ...
        document.addEventListener('keydown', function(e) {
            if (e.altKey && e.key === 'a') {
                e.preventDefault();
                window.location.href = '{{ route("motorbikes.create") }}';
            }
            if (e.altKey && e.key === 's') {
                e.preventDefault();
